...
export default (io: Server, socket: Socket) => {
    socket.on(ON_BET_COIN, async ({ roomId, playerId, betting }: { roomId: string; playerId: string; betting: number }) => {
        if (!playerId || socket.data.playerId !== playerId) { socket.emit(EMIT_BETTING, { status: ERROR }); return; }

        const playerState = await Game.bet(roomId, playerId, betting);
    ...
    socket.on(ON_FOLD_CARD, async ({ roomId, playerId }: { roomId: string; playerId: string }) => {
        if (!playerId || socket.data.playerId !== playerId) { socket.emit(EMIT_FOLDING, { status: ERROR }); return; }

        const playerState = await Game.fold(roomId, playerId);
