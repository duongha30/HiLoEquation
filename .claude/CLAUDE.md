...
**Game finalization**
- Server handler for `FINISH_GAME { roomCode, playerId, result }`: calls `setSubmission` to record score then `finalizeRound`, which awards the entire `totalBetting` pot to the player with the highest score and resets bets/cards.
